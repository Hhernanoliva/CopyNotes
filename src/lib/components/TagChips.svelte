...
{#each tags as tag (tag.id)}
	...
	>
		...
		<!-- Sin `onRemove` la etiqueta se ve pero no se saca: es lo que necesita una
		     nota que te compartieron, donde las etiquetas son parte de lo que estás
		     leyendo y no algo tuyo para tocar. El candado vive acá y no en cada
		     llamador porque una cruz que llama a nada revienta en pantalla. -->
		{#if onRemove}
			<button
				type="button"
