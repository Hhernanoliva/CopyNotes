...
- **Descargar respaldo completo**: baja un archivo con todas tus notas, bloques y preferencias. Es el archivo que después podés importar para restaurar todo. El nombre incluye fecha y hora (ej. `copynotes-backup-2026-07-10-1630.json`). Si alguna de tus notas tiene una captura de pantalla pegada —aunque esté en la papelera—, el archivo sale con la terminación `.copynotes` en lugar de `.json`: es un ZIP común, de esos que se abren solos con doble clic en Mac y en Windows. Adentro encontrás `backup.json` —el mismo texto legible de siempre—, un `README.txt` que explica qué es cada cosa, y las capturas. Sin capturas, el archivo es exactamente el `.json` de toda la vida. Si por algún motivo a una captura le faltan los datos, el respaldo se baja igual y el mensaje te lo dice: *"Se guardó el respaldo, pero le falta alguna imagen"* — un respaldo al que le falta una foto y lo avisa vale más que uno que se declara completo.
...
1. Tocá **Elegir archivo de respaldo…** y seleccioná un archivo de CopyNotes: sirven los dos, el `.json` de siempre y el `.copynotes` con capturas adentro. Si le cambiaste el nombre al archivo no importa: CopyNotes mira lo que hay adentro, no cómo se llama. Si te equivocás de archivo y elegís uno enorme —un video, por ejemplo—, ni lo abre: te avisa que pesa demasiado y no hace nada. Y si el archivo simplemente no es un respaldo, te lo dice y **no toca nada** de lo tuyo.
