...
- Escribí **`/`** en cualquier parte del renglón (vacío, con texto, o en medio del texto): se abre un menú amplio con los tipos. **H1, H2 y H3 comparten una sola fila** para dejar más opciones a la vista. Filtrá escribiendo (ej. `/ta` para Tarea), movete con **↑ ↓**, elegí con **Enter** o **Tab** y cancelá con **Escape**.
...
- El menú se abre **debajo de la `/` que tipeaste**, no al principio del renglón: en un renglón largo aparecía lejos de donde estabas escribiendo. Mientras filtrás se queda quieto, y no se pasa del borde derecho de la pantalla.
- El menú **queda pegado al renglón que lo abrió**: si scrolleás la nota con el menú abierto, se mueve con el renglón en vez de quedarse clavado en la pantalla. Y si el renglón está cerca del borde de abajo y el menú no entra, **se da vuelta y sale arriba**, para que se vea entero.
- **En el celular funciona igual**, con el teclado del teléfono. Antes no: si el renglón ya tenía texto, la `/` se escribía y el menú no aparecía nunca (en un renglón vacío sí). La causa era el corrector del teclado, que confirma o corrige la palabra anterior en el mismo momento en que llega la `/`, y CopyNotes lo confundía con texto pegado. Lo mismo pasaba con el `#` de las etiquetas.
